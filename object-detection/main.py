...
def get_food_metrics(client, food_items):
    ...
    chat_res = client.chat.completions.create(
        messages=[{
            ...
            "content": [{
                "type": "text",
                "text": f"Please provide an array of objects for the following food items: {food_items_str}. Each object should contain 'food_item', 'calories', 'carbohydrates', 'proteins', 'sodium', and 'fats' as fields, in JSON format."
            }],
        }],
        model="llava-v1.5-7b-4096-preview",
    )
...
while retry_count < max_retries:
    ...
    if processed_result != "No food content found!":
        ...
        if food_items_list:
            food_metrics = get_food_metrics(client, food_items_list)
